...
import { createPosition, getPositionById, updatePosition } from '@/api/position'
...
const loadData = async () => {
  ...
  try {
    const { data } = await getPositionById(editingId.value)
    if (data) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!isEdit.value) {
      await createPosition(payload as API.CreatePositionDto)
      ElMessage.success('新增成功')
    } else {
      await updatePosition(editingId.value!, payload as API.UpdatePositionDto)
...
defineExpose({
  show: (dialogTitle: string, positionId?: string) => {
