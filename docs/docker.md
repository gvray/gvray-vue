...
Dockerfile 通过 `BUILD_ENV` 构建参数选择目标环境，对应 `pnpm build:dev` /
...
# 构建预发布环境镜像
./docker/scripts/build.sh -e staging

# 构建生产环境镜像（默认）
./docker/scripts/build.sh -e prod
```

> 说明：vite 的 `outDir` 固定为
> `dist/`，所有环境构建产物均输出到该目录；`BUILD_ENV` 仅用于选择
> `pnpm build:${BUILD_ENV}` 命令与对应的 `.env.${BUILD_ENV}` 配置。
...
# 容器配置
export CONTAINER_NAME=gvray-vue-web
...
- 生产环境（`.env.prod`）中 `APP_API_URL` 为后端完整地址（如
  `https://api.gvray.com`），前端直连后端，无需 Nginx 代理
- 若需在同一容器内代理后端，取消 `docker/default.conf` 中 `location /api/`
  的注释并配置 `proxy_pass`
...
# 查看最近 100 行日志
docker logs --tail=100 gvray-vue-web
...
```bash
# 进入容器 shell
docker exec -it gvray-vue-web sh
```
...
```bash
# 查看容器日志
docker logs gvray-vue-web

# 检查容器状态
docker inspect gvray-vue-web

# 查看健康检查状态
docker inspect --format='{{.State.Health.Status}}' gvray-vue-web
```
...
# 修改端口映射
docker run -p 8888:80 gvray-vue:latest
```
...
| `docker exec -it gvray-vue-web sh` | 进入容器          |
| `docker stats gvray-vue-web`       | 查看资源使用      |
...
- [Vite 构建文档](https://vite.dev/guide/build.html)
...
2. **API 代理配置**：
   ...
   - `.env.prod` 中 `APP_API_URL` 为后端完整地址，前端直连后端
   - 如需代理后端 API，修改 `docker/default.conf` 中的 `upstream` 配置并取消
     `location /api/` 注释

3. **构建命令**：Dockerfile 默认执行 `pnpm build:prod`，通过
   `--build-arg BUILD_ENV=staging` 可切换为其他环境构建
